(function () {
  ...
  function renderBlock(block) {
    ...
    if (block.type === "network") return `<div class="knowledge-network" role="img" aria-label="家族关系网络"><svg class="knowledge-network-connections" aria-hidden="true"></svg>${block.nodes.map((node, index) => `<div class="knowledge-network-node${index === 0 ? " primary" : ""}">${escapeHtml(node)}</div>`).join("")}</div>`;
  ...
  function render(article) {
    ...
    `;
    requestAnimationFrame(renderNetworkConnections);
  ...
  function renderNetworkConnections() {
    articleRoot.querySelectorAll(".knowledge-network").forEach((network) => {
      const svg = network.querySelector(".knowledge-network-connections");
      const primary = network.querySelector(".knowledge-network-node.primary");
      const nodes = [...network.querySelectorAll(".knowledge-network-node:not(.primary)")];
      if (!svg || !primary || !nodes.length) return;

      const networkRect = network.getBoundingClientRect();
      if (!networkRect.width || !networkRect.height) return;
      const primaryRect = primary.getBoundingClientRect();
      const startX = primaryRect.left + primaryRect.width / 2 - networkRect.left;
      const startY = primaryRect.bottom - networkRect.top;
      ...
      svg.innerHTML = nodes.map((node) => {
        ...
        const endY = nodeRect.top - networkRect.top;
  ...
  window.addEventListener("resize", renderNetworkConnections);
